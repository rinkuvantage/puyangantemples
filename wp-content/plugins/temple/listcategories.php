...
{ 
	...
	if(isset($_REQUEST['pagedid']) && $_REQUEST['pagedid']>1)
	{
		...
		$limitCategoryt=$totalrec*($pageid-1);
	}
	else
	{
		$pageid=1;
	...
	$querystr = "SELECT * FROM ".$prefix."temple_categories $where limit $limitCategoryt, $totalrec";
	$states = $wpdb->get_results($querystr, OBJECT);
	
	$querystr = "SELECT * FROM ".$prefix."temple_categories $where";
...
<?php $url=get_option('home').'/wp-admin/admin.php?page=ManageCategories'; ?>
<div class="wrap">
<div style="margin:15px;">
<?php  echo "<div style='color: #222; font-size: 1.5em; font-weight: 400; margin: 0.83em 0; float:left;'>" . __( 'Manage Categories', 'webserve_trdom' ) . "</div>"; ?>
<a href="<?php _e(get_option('home')); ?>/wp-admin/admin.php?page=AddCategory"><div class="add-new" style="margin: 0.83em 0.5em;">Add Category</div></a>
</div>
...
<?php if(count($states)>0){ ?>
	...
	<?php $cnt=$limitCategoryt+1; foreach($states as $st){ ?>
	  <tr>
		<td valign="top" align="left" style="border-top:1px solid #ccc;">&nbsp;<?php _e($cnt); ?></td>
		<td valign="top" align="left" style="border-top:1px solid #ccc; border-left:1px solid #ccc;"><?php _e($st->category); ?></td>
		<td valign="top" align="left" style="border-top:1px solid #ccc; border-left:1px solid #ccc;">
			<a href="<?php _e($url); ?>&ho=editcategory&id=<?php _e($st->id); ?>">View and Edit</a>&nbsp;&nbsp;
			<a href="javascript:if(confirm('Please confirm that you would like to delete this?')) {window.location='<?php _e($url); ?>&ho=deletecategory&id=<?php _e($st->id); ?>';}">Delete</a>
		</td>
	  </tr>
	  <?php $cnt++; } ?>
	</table>
 <?php 
	...
	{
		echo "<strong>No Category Found!</strong>" ;
	}
	?>    
</form>
