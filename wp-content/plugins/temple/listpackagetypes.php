...
{ 
	...
	$querystr = "SELECT * FROM ".$prefix."temple_package_types $where limit $limitstart, $totalrec";
	$states = $wpdb->get_results($querystr, OBJECT);
	
	$querystr = "SELECT * FROM ".$prefix."temple_package_types $where";
...
<?php  echo "<div style='color: #222; font-size: 1.5em; font-weight: 400; margin: 0.83em 0; float:left;'>" . __( 'Manage Package Types', 'webserve_trdom' ) . "</div>"; ?>
<a href="<?php _e(get_option('home')); ?>/wp-admin/admin.php?page=AddPackage"><div class="add-new" style="margin: 0.83em 0.5em;">Add Package Type</div></a>
</div>
...
<?php if(count($states)>0){ ?>
	...
	<?php $cnt=$limitstart+1; foreach($states as $st){ ?>
	  <tr>
		...
		<?php _e($st->package_type); ?>
		<?php if(trim($st->image)!='' && file_exists('../wp-content/uploads/packages/'.$st->image)){ ?>
			<br class="clr" />
			<img src="<?php echo get_option('home').'/wp-content/uploads/packages/'.$st->image; ?>" width="100" />
			<?php } ?>
		</td>
		<td valign="top" align="left" style="border-top:1px solid #ccc; border-left:1px solid #ccc;">
			<a href="<?php _e($url); ?>&ho=editpackagetype&id=<?php _e($st->id); ?>">View and Edit</a>&nbsp;&nbsp;
			<a href="<?php _e($url); ?>&ho=listpackages&id=<?php _e($st->id);?>">Manage Packages</a>&nbsp;&nbsp;
			<a href="javascript:if(confirm('Please confirm that you would like to delete this case?')) {window.location='<?php _e($url); ?>&ho=deletepackagetype&id=<?php _e($st->id); ?>';}">Delete</a>
		</td>
	  </tr>
	  <?php $cnt++; } ?>
	</table>
