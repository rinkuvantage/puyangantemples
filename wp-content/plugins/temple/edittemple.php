...
if(isset($_POST['registration']))
{
	...
	if(count($error)<=0)
	{
		...
		$sql="UPDATE `".$prefix."temple_temples` set state_id='$state_id', city_id='$city_id', district_id='$district_id', name='$name', inthetemple='$inthetemple', openinghours='$openinghours', address='$address', googleaddress='$googleaddress', phone='$phone', general_info='$general_info', description='$description', heighlight='$heighlight', latitude='$latitude', longitude='$longitude', location='$location', nearrailway='$nearrailway', nearairport='$nearairport', accomodation='$accomodation', star_id='$star_id', category_id='$category_id' where id='$id'";
		$result = $wpdb->query( $sql );
		
		$sql="DELETE FROM `".$prefix."temple_templesgod` where temple_id='$temple_id'"; 
		$result = $wpdb->query( $sql );
		
		$sql="UPDATE `".$prefix."posts` set post_title='$name' where ID='$post_id'";
		$result = $wpdb->query( $sql );
		...
		$packs=$_POST['packs'];
		$sql="DELETE FROM `".$prefix."temple_package_price` where temple_id='$temple_id'"; 
		$result = $wpdb->query( $sql );
		...
		$url=get_option('home').'/wp-admin/admin.php?page=Temples&update=succ';
...
jQuery(document).ready(function(){
	...
	var packtype_id=jQuery('#package_type_id').val();
	...
	if(jQuery.trim(state_id)!=''){
		var url='<?php echo get_option('home');?>/wp-content/plugins/temple/finddistricts.php';
		jQuery.post(url, {state_id:state_id,selecteddistrict:'<?php echo $district_id; ?>'},
		function(data)
		{
			jQuery('#districtdiv').html(data);
			var district_id=jQuery('#district_id').val();
			if(jQuery.trim(district_id)!=''){
				var url='<?php echo get_option('home');?>/wp-content/plugins/temple/findCity.php';
				jQuery.post(url, {district_id:district_id,selectedcity:'<?php echo $city_id; ?>'},
				function(data)
				{
					jQuery('#citydiv').html(data);
				});
        ...
        <div class="tabcontents donotshowerror">
        	<div id="view1">
				...
				<div class="e-mail">
                    ...
                    <div class="field"><input type="file"<?php if(trim($image)=='' || !file_exists('../wp-content/uploads/temples/'.$image)){ ?> class="required"<?php } ?> name="image"/>
					<?php if(trim($image)!='' && file_exists('../wp-content/uploads/temples/'.$image)){ ?>
					<br class="clr" />
					<img src="<?php echo get_option('home').'/wp-content/uploads/temples/'.$image; ?>" width="100" />
					<?php } ?>
					</div>
                </div>
				<div class="e-mail">
					...
					?>	
                    	<input type="checkbox" value="<?php _e($temgod->id); ?>"<?php if(count($tempgod)>0){_e(' checked="checked"');} ?> name="god_id[]"/><?php _e($temgod->god); ?>
						<br/>
					<?php		
						}
					?>
			...
			<div id="view2">
    			...
    			<div class="e-mail">
                    ...
                    <div class="field"><input type="text" name="accomodation_name[]" value="<?php  _e($accomodats['accomodation_name'][$i]); ?>"/></div>
                </div>
				<div class="e-mail">
                    <div class="adress">Phone:  </div>
                    <div class="field"><input type="text" name="accomodation_phone[]" value="<?php  _e($accomodats['accomodation_phone'][$i]); ?>"/></div>
                </div>
				<div class="clr"></div>
				<?php } ?>
        	</div>
			...
			<div id="view4">
				...
				<div class="e-mail package pack_<?php _e($package->package_type_id); ?>">
                    ...
                    <div class="field">
						<input type="text" name="price_<?php _e($package->id); ?>" value="<?php echo $templeprices[0]->price; ?>" class="number" placeholder="price" />
