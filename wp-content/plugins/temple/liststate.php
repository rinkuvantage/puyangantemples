...
{ 
	...
	$where=" order by state";
	$querystr = "SELECT * FROM ".$prefix."temple_state $where limit $limitstart, $totalrec";
	$states = $wpdb->get_results($querystr, OBJECT);
	
	$querystr = "SELECT * FROM ".$prefix."temple_state $where";
...
<?php  echo "<div style='color: #222; font-size: 1.5em; font-weight: 400; margin: 0.83em 0; float:left;'>" . __( 'Manage State', 'webserve_trdom' ) . "</div>"; ?>
<a href="<?php _e(get_option('home')); ?>/wp-admin/admin.php?page=AddState"><div class="add-new" style="margin: 0.83em 0.5em;">Add State</div></a>
</div>
...
<?php if(count($states)>0){ ?>
	<table width="100%" align="center" border="0" cellpadding="0" cellspacing="0" style="border:1px solid #ccc;">
		<tr>
            ...
            <th valign="top" align="left" style="border-left:1px solid #ccc;"><?php _e("Add New" ); ?></th>
		</tr>
	<?php $cnt=$limitstart+1; foreach($states as $st){ ?>
	  <tr>
		<td valign="top" align="left" style="border-top:1px solid #ccc;">&nbsp;<?php _e($cnt); ?></td>
		<td valign="top" align="left" style="border-top:1px solid #ccc; border-left:1px solid #ccc;"><?php _e($st->state); ?></td>
		<td valign="top" align="left" style="border-top:1px solid #ccc; border-left:1px solid #ccc;">
			<a href="<?php _e($url); ?>&ho=stateedit&id=<?php _e($st->id); ?>">View and Edit</a>&nbsp;&nbsp;
			<a href="<?php _e($url); ?>&ho=listdistricts&state_id=<?php _e($st->id); ?>">Manage Districts</a>&nbsp;&nbsp;
			<a href="<?php _e($url); ?>&ho=listcity&state_id=<?php _e($st->id); ?>">Manage Cities</a>&nbsp;&nbsp;
			<a href="javascript:if(confirm('Please confirm that you would like to delete this?')) {window.location='<?php _e($url); ?>&ho=statedelete&id=<?php _e($st->id); ?>';}">Delete</a>
		</td>
        <td valign="top" align="left" style="border-top:1px solid #ccc; border-left:1px solid #ccc;">
			<a href="<?php _e($url); ?>&ho=addcity&state_id=<?php _e($st->id); ?>"><div class="fl add-new">Add City</div></a>
			<a href="<?php _e($url); ?>&ho=adddistrict&state_id=<?php _e($st->id); ?>"><div class="add-new fl ml10">Add District</div></a>
        </td>
	  </tr>
	  <?php $cnt++; } ?>
	</table>
 <?php 
	...
	{
		echo "<strong>No State Found!</strong>" ;
	}
	?>    
</form>
