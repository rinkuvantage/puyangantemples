<?php 
	...
	$where=" where id!='' $cnd order by id desc";
	$querystr = "SELECT * FROM ".$prefix."temple_gallery_images $where limit $limitstart, $totalrec";
	$cities = $wpdb->get_results($querystr, OBJECT);
	
	$querystr = "SELECT * FROM ".$prefix."temple_gallery_images $where";
...
<?php  echo "<div style='color: #222; font-size: 1.5em; font-weight: 400; margin: 0.83em 0; float:left;'>" . __( 'Manage Gallery', 'webserve_trdom' ) . "</div>"; ?>
<a href="<?php _e($url); ?>&ho=addgallery<?php _e($ptypeurl); ?>"><div class="add-new" style="margin: 0.83em 0.5em;">Add Photos</div></a>
</div>
...
<?php if(count($cities)>0){ ?>
	...
	?>
	  <tr>
		...
		<?php if(trim($city->image)!='' && file_exists('../wp-content/uploads/temples/gallery/thumb/'.$city->image)){ ?>
			<img src="<?php echo get_option('home').'/wp-content/uploads/temples/gallery/thumb/'.$city->image; ?>" width="100" />
		<?php } ?>
		</td>
		<td valign="top" align="left" style="border-top:1px solid #ccc; border-left:1px solid #ccc;">
			<a href="<?php _e($url); ?>&ho=editgallery&id=<?php _e($city->id); ?>">View and Edit</a>&nbsp;&nbsp;
			<a href="javascript:if(confirm('Please confirm that you would like to delete this?')) {window.location='<?php _e($url); ?>&ho=deletegallery&id=<?php _e($city->id); ?>';}">Delete</a>
        </td>
	  </tr>
	  <?php $cnt++; } ?>
	</table>
	...
	{
		echo "<strong>No Gallery Image Found!</strong>" ;
	}
	?>
</form>
