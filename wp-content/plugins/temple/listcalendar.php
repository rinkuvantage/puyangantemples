<?php 
	...
	$cnd='';
	$ptypeurl='';
	if(isset($_REQUEST['temple_id']) && trim($_REQUEST['temple_id'])!='')
	{
		$temple_id=$_REQUEST['temple_id'];
		$cnd.=" and temple_id = '$temple_id'";
		$ptypeurl='&temple_id='.$temple_id;
	}
	
	$where=" where id!='' $cnd order by cdate desc";
	$querystr = "SELECT * FROM ".$prefix."temple_calendar $where limit $limitstart, $totalrec";
	$cities = $wpdb->get_results($querystr, OBJECT);
	
	$querystr = "SELECT * FROM ".$prefix."temple_calendar $where";
...
<?php  echo "<div style='color: #222; font-size: 1.5em; font-weight: 400; margin: 0.83em 0; float:left;'>" . __( 'Manage Calendar', 'webserve_trdom' ) . "</div>"; ?>
<a href="<?php _e($url); ?>&ho=addcalendar<?php _e($ptypeurl); ?>"><div class="add-new" style="margin: 0.83em 0.5em;">Add New</div></a>
</div>
...
<?php if(count($cities)>0){ ?>
	<table width="100%" align="center" border="0" cellpadding="0" cellspacing="0" style="border:1px solid #ccc;">
		<tr>
            ...
            <th valign="top" align="left" style="border-left:1px solid #ccc;"><?php _e("Date" ); ?></th>
			...
			<th valign="top" align="left" style="border-left:1px solid #ccc;"><?php _e("Worship" ); ?></th>
	...
	?>
	  <tr>
		...
		<td valign="top" align="left" style="border-top:1px solid #ccc; border-left:1px solid #ccc;"><?php _e(date('d, M Y', strtotime($city->cdate))); ?></td>
        <td valign="top" align="left" style="border-top:1px solid #ccc; border-left:1px solid #ccc;">
		<?php if(trim($city->image)!='' && file_exists('../wp-content/uploads/temples/calendar/'.$city->image)){ ?>
			<img src="<?php echo get_option('home').'/wp-content/uploads/temples/calendar/'.$city->image; ?>" width="100" />
		<?php } ?>
		</td>
		<td valign="top" align="left" style="border-top:1px solid #ccc; border-left:1px solid #ccc;"><?php _e($city->special); ?></td>
		<td valign="top" align="left" style="border-top:1px solid #ccc; border-left:1px solid #ccc;"><?php _e($city->worship); ?></td>
		<td valign="top" align="left" style="border-top:1px solid #ccc; border-left:1px solid #ccc;">
			<a href="<?php _e($url); ?>&ho=editcalendar&id=<?php _e($city->id); ?>">View and Edit</a>&nbsp;&nbsp;
			<a href="javascript:if(confirm('Please confirm that you would like to delete this?')) {window.location='<?php _e($url); ?>&ho=deletecalendar&id=<?php _e($city->id); ?>';}">Delete</a>
        </td>
	  </tr>
	  <?php $cnt++; } ?>
	</table>
	...
	{
		echo "<strong>No Calendar Found!</strong>" ;
	}
	?>
</form>
